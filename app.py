...
def tocar_som_notificacao():
    ...
    for i in range(int(sample_rate * 0.15)):
        ...
        value = int(32767 * 0.3 * envelope * math.sin(2 * math.pi * 659.25 * t))
...
st.markdown(
    ...
    label, .stRadio label, .stTextArea label, .stFileUpload label, .stTextInput label, .stSelectbox label {
        color: #FFFFFF !important;
    ...
    .stTextArea textarea:focus, .stTextInput input:focus {
        ...
        box-shadow: 0 0 1px #E2001A !important;
    }
...
with col_direita:
    ...
    if btn_iniciar:
        ...
        st.warning(
            f"⚠️ **Atenção:** Consulta anterior foi interrompida ou concluída. Foram recuperados **{len(df_cache)}** itens."
        )
        ...
        st.markdown(
            ...
            <div class="latam-card">
                <div class="latam-card-title">🚀 Nosso Propósito</div>
                <div class="latam-quote">
                    "Levar os sonhos ao seu destino com segurança, eficiência e agilidade – otimizando processos fiscais."
                </div>
            </div>
            """,
            unsafe_allow_html=True,
...
if btn_enviar_feedback:
    if not mensagem.strip():
        ...
        try:
            resposta = requests.post("https://formspree.io/f/mrenybwd", data=dados_envio)
